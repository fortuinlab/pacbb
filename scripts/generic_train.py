...
from core.distribution.utils import from_copy, from_random, from_zeros
...
def main(config: dict, config_path: str):
    ...
    objective = objective_factory.create(
        config["factory"]["prior_objective"]["name"],
    ...
    posterior = from_copy(dist=prior, distribution=GaussianVariable, requires_grad=True)
