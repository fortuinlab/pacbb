...
def test_gaussian_distribution():
    ...
    scaled_rho = torch.log(torch.exp(scaled_sigma) - 1.)
    ...
    assert distribution3.compute_kl(distribution1).item() - distribution3.compute_kl(distribution2).item() <= 1e-6
    assert distribution4.compute_kl(distribution1).item() - distribution3.compute_kl(distribution2).item() >= 1e-6
