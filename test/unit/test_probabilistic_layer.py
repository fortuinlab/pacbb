...
def test_probabilistic_linear_layer():
    ...
    assert torch.allclose(layer1.weight_prior.mu, torch.zeros_like(layer1.weight_prior.mu))
    assert torch.allclose(layer1.weight_prior.rho, torch.ones_like(layer1.weight_prior.rho) * layer1._rho)
    assert torch.allclose(layer1.bias_prior.mu, torch.zeros_like(layer1.bias_prior.mu))
    assert torch.allclose(layer1.bias_prior.rho, torch.ones_like(layer1.bias_prior.rho) * layer1._rho)
    assert torch.allclose(layer1.bias.mu, torch.zeros_like(layer1.bias.mu))
    assert torch.allclose(layer1.bias.rho, torch.ones_like(layer1.bias.rho) * layer1._rho)
