...
class AbstractObjective(ABC):
    ...
    def train_objective(
        ...
        loss_ce, loss_01, outputs = RiskEvaluator.compute_losses_old(model, data, target.long(), self._clamping, self._pmin)
    ...
    def compute_risks(self, model: AbstractPBPModel, data: Tensor, targets: Tensor, num_samples: int, num_samples_bound: int) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        avg_loss_ce, avg_loss_01 = RiskEvaluator.compute_avg_losses_old(self._num_mc_samples, model, data, targets, True, self._pmin)
